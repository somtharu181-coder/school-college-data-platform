...
def scrape_college_slugs(
    ...
    try:
        ...
        logger.info(
            "College slug scrape: %s institutions across %s pages "
    ...
    for page_num, results, pagination_meta in paginate(
        ...
        if pages_crawled % progress_every == 0:
            ...
            logger.info(
                "College progress: page %d/%s — %d slugs collected so far",
                page_num, total_pages_actual, len(slugs),
            )

    logger.info(
        "College slug scrape complete — %d pages crawled, %d slugs collected.",
...
def _extract_slugs(
    ...
    for item in results:
        ...
        if not slug:
            logger.warning(
                "College result missing slug field: %s",
                str(item)[:120],
            )
            continue

        if not isinstance(slug, str) or not slug.strip():
            logger.warning("Skipping college result with invalid slug: %r", slug)
            continue
